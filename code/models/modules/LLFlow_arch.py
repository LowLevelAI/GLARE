...
class LLFlow(nn.Module):
    def __init__(self, in_nc=3, out_nc=3, nf=32, nb=4, gc=32, scale=4, K=None, opt=None, step=None):
        ...
        self.flowUpsamplerNet = \
            FlowUpsamplerNet((self.crop_size, self.crop_size, 32), hidden_channels, K,
                             flow_coupling=opt['network_G']['flow']['coupling'], opt=opt)       #coupling: CondAffineSeparatedAndCond
        self.i = 0
        if self.opt['to_yuv']:               #false
        ...
        if self.opt['align_maxpool']:   #align_maxpool: true
    ...
    def normal_flow(self, gt, lr, y_onehot=None, epses=None, lr_enc=None, add_gt_noise=True, step=None,
        ...
        if lr_enc is None and self.RRDB:
            lr_enc = self.rrdbPreprocessing(lr)    #执行
        #print('conditionalencoder',lr_enc['color_map'].shape)   #[2,3,160,160]
        ...
        #print(epses.shape)  torch.Size([2, 192, 20, 20])
        ...
        if 'avg_color_map' in self.opt.keys() and self.opt['avg_color_map']:
            ...
            if self.RRDB is not None:
                ...
                #print(gt.sum(dim=1, keepdims=True).shape)
            else:
    ...
    def rrdbPreprocessing(self, lr):
        rrdbResults = self.RRDB(lr, get_steps=True)     #得到encoder 之后的feature--dict
