...
class FlowStep(nn.Module):
    ...
    def normal_flow(self, z, logdet, rrdbResults=None):
        ...
        if self.norm_type == "ConditionalActNormImageInjector":
            ...
            z, logdet = self.actnorm(z, logdet=logdet, reverse=False)   #检查维度
        ...
        # 3. coupling
        if need_features or self.flow_coupling in ["condAffine", "condFtAffine", "condNormAffine"]:   #True for layer 3-6
            img_ft = getConditional(rrdbResults, self.position)   #把conditional encoder的对应位置的feature拿下来
