...
class VQGANConEncoder(nn.Module):
    def __init__(self, in_nc, out_nc, nf, nb, gc=32, scale=4, opt=None):
        ...
        self.fine_tune_color_map = nn.Sequential(nn.Conv2d(nf, 32, 1, 1),nn.Sigmoid())   #输出channel改为32，为了配合VQGAN encoder feature
    ...
    def forward(self, x, get_steps=False):
        ...
        fea_head = F.max_pool2d(fea, 2)  #第一次下采样
        print(fea_head.shape)
        ...
        fea_down2 = fea_head + trunk    #residual block

        fea_down4 = self.downconv1(F.interpolate(fea_down2, scale_factor=1 / 2, mode='bilinear', align_corners=False,
                                                 recompute_scale_factor=True))# 第二次下采样
        fea = self.lrelu(fea_down4)

        fea_down8 = self.downconv2(
            F.interpolate(fea, scale_factor=1 / 2, mode='bilinear', align_corners=False, recompute_scale_factor=True))   #第三次下采样
