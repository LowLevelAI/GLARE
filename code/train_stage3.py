...
from data.LoL_dataset import LoL_Dataset
...
def main():
    ...
    parser.add_argument('--opt', type=str, help='Path to option YMAL file.',
                            default='./code/confs/train_stage3_LOL.yml') 
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            nll, nll_percep, nll_l1, nll_ssim = model.optimize_parameters(current_step, val_loader)
            ...
            if current_step % opt['logger']['print_freq'] == 0 \
                ...
                message = '<epoch:{:3d}, iter:{:8,d}, lr:{:.3e}, t:{:.2e}, td:{:.2e}, eta:{:.2e}, nll:{:.3e}, nll_percep:{:.3e}, nll_l1:{:.3e}, nll_ssim:{:.3e}> '.format(
                    epoch, current_step, model.get_current_learning_rate(), avg_time, avg_data_time,
                    eta(avg_time), nll, nll_percep, nll_l1, nll_ssim)
                print(message)
            timer.tick()
            # Reduce number of logs
            if current_step % 5 == 0:
                tb_logger_train.add_scalar('loss/nll', nll, current_step)
                tb_logger_train.add_scalar('loss/nll_percep', nll_percep, current_step)
                tb_logger_train.add_scalar('loss/nll_l1', nll_l1, current_step)
                tb_logger_train.add_scalar('loss/nll_ssim', nll_ssim, current_step)
            ...
            save_ = False
            if current_step >= 20000 and current_step % 100 == 0:
                save_ = True
            if current_step >= 30000 and current_step % 50 == 0:
                save_ = True
            if current_step >= 36000 and current_step % 10 == 0:
                save_ = True
